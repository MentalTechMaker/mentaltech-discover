...
export const PublicSubmissionsAdmin: React.FC<Props> = ({
  onCreateProduct,
}) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {toast && (
        <div
          className={`fixed bottom-6 right-6 z-[300] flex items-center gap-3 px-4 py-3 rounded-xl shadow-xl text-sm font-semibold transition-all ${
            toast.type === "success"
      ...
      {!loading && activeTab === "submissions" && (
        <div className="space-y-2">
          ...
          {filteredSubmissions.map((sub) => {
            const st = STATUS_BADGES[sub.status] ?? {
              label: sub.status,
              cls: "bg-gray-100 text-gray-600",
            };
              ...
              >
                ...
                {(canReject || sub.collectifRequested) && (
                  <div className="px-4 pb-2 flex gap-3 border-t border-gray-100 pt-2">
                      ...
                      <>
                        <button
                          onClick={(e) => {
                            ...
                            withLoading(async () => {
                              refreshSub(
                                await approvePublicSubmission(sub.id, {}),
                              );
                              showToast("Soumission approuvée");
                            });
                          }}
                        ...
                        {sub.status === "submitted" && (
                          <button
                            onClick={(e) => {
                              ...
                              withLoading(async () => {
                                refreshSub(
                                  await markSubmissionUnderReview(sub.id),
                                );
                                showToast("Mise en review");
                              });
                            }}
                            disabled={actionLoading}
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setRejectingSubId(isRejecting ? null : sub.id);
                            setAdminNotes("");
                            setCollectifSubId(null);
                          }}
                    ...
                    {sub.collectifRequested &&
                      !["accepted", "refused"].includes(
                        sub.collectifStatus,
                      ) && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setCollectifSubId(isCollectif ? null : sub.id);
                            setHelloassoUrl("");
                            setRejectingSubId(null);
                          }}
                  ...
                  <div className="px-4 pb-4 pt-2 space-y-2 border-t border-red-100 bg-red-50">
                    ...
                    <button
                      onClick={() =>
                        withLoading(async () => {
                          refreshSub(
                            await rejectPublicSubmission(sub.id, adminNotes),
                          );
                          setRejectingSubId(null);
                      ...
                      <button
                        onClick={() =>
                          withLoading(async () => {
                            ...
                            showToast("Statut: En discussion");
                          })
                        }
                        disabled={actionLoading}
                          ...
                          withLoading(async () => {
                            refreshSub(
                              await updateCollectifStatus(
                                ...
                                "accepted",
                                undefined,
                                helloassoUrl || undefined,
                              ),
                            );
                            setCollectifSubId(null);
                            showToast("Collectif accepté");
                          })
                        }
                        disabled={actionLoading}
                      ...
                      <button
                        onClick={() =>
                          withLoading(async () => {
                            ...
                            setCollectifSubId(null);
                            showToast("Collectif refusé", "error");
                          })
                        }
                        disabled={actionLoading}
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex flex-col items-end gap-1 flex-shrink-0">
                  ...
                  />
                  {app.isCollectiveMember && (
                    <Badge label="Membre" cls="bg-amber-100 text-amber-700" />
                  )}
                </div>
              </div>
            </div>
              ...
              <div className="flex gap-2">
                <button
                  onClick={() =>
                    withLoading(async () => {
                      refreshApp(
                        await acceptHealthProApplication(
                          selectedApp.id,
                          helloassoUrl || undefined,
                ...
                <button
                  onClick={() =>
                    withLoading(async () => {
                      refreshApp(
                        await refuseHealthProApplication(
                          selectedApp.id,
                          adminNotes || undefined,
                        ),
                      );
            ...
            <label className="flex items-center gap-2 cursor-pointer border border-gray-200 rounded-xl p-3 bg-white">
              <input
                ...
                onChange={() =>
                  withLoading(async () => {
                    refreshApp(
                      await setHealthProCollectiveMember(
                        selectedApp.id,
                        !selectedApp.isCollectiveMember,
                      ),
                    );
                    showToast(
                      !selectedApp.isCollectiveMember
