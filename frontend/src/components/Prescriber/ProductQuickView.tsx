      ...
      >
        ...
        <div className="flex items-start gap-3 p-5 border-b border-gray-100">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="font-bold text-text-primary text-lg leading-tight truncate">{product.name}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          {/* Tagline */}
          <p className="text-primary font-semibold text-base leading-snug">{product.tagline}</p>
          ...
          {product.scoreLabel && (
            <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
              ...
              <div>
                <p className="font-semibold text-text-primary text-sm">{label.text}</p>
                {product.scoreTotal != null && (
                  <p className="text-xs text-text-secondary">{product.scoreTotal}/100 points</p>
                )}
              </div>
            </div>
          )}
          ...
          {product.scoring && (
            <div className="space-y-2">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Évaluation détaillée</p>
                ...
                return (
                  <div key={key}>
                    <div className="flex justify-between items-center mb-0.5">
                      <span className="text-xs text-text-primary">{criteriaLabel}</span>
                      <span className="text-xs font-bold text-text-primary">{score}/20</span>
                    </div>
          ...
          {product.audience && product.audience.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Public cible</p>
              <div className="flex flex-wrap gap-1.5">
                {product.audience.map((a) => (
                  <span key={a} className="px-2.5 py-1 bg-blue-50 text-blue-700 text-xs rounded-full font-medium">{AUDIENCE_LABELS[a] ?? a}</span>
                ))}
              </div>
            </div>
          )}
          ...
          {product.pricing?.model && (
            <div className="p-3 bg-gray-50 rounded-lg flex items-center gap-3">
              <span className="text-lg">{product.pricing.model === 'free' ? '💚' : '💳'}</span>
              <div>
                <p className="text-sm font-semibold text-text-primary capitalize">{product.pricing.model}</p>
                {product.pricing.amount && (
                  <p className="text-xs text-text-secondary">{product.pricing.amount}</p>
                )}
              </div>
            </div>
          )}
        ...
        <div className="p-5 border-t border-gray-100">
          ...
          >
            {isSelected ? 'Retirer de la prescription' : 'Ajouter à la prescription'}
          </button>
        </div>
      </div>
