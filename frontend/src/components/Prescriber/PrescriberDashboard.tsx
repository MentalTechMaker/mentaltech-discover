...
export const PrescriberDashboard: React.FC = () => {
  ...
  const handleRenew = async (id: string) => {
    if (
      !window.confirm(
        "Renouveler cette prescription pour 30 jours supplémentaires ?",
      )
    )
      return;
    setRenewingId(id);
  ...
  const handleRemoveFavorite = async (productId: string) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Erreur lors du retrait du favori",
      );
    } finally {
  ...
  const handleDeleteNote = async (productId: string) => {
    if (!window.confirm("Supprimer cette note clinique ?")) return;
    setDeletingNoteId(productId);
    try {
      await apiDeleteNote(productId);
      setNotes((prev) => prev.filter((n) => n.productId !== productId));
    } catch (err) {
      setError(
        err instanceof Error
          ? err.message
    ...
    return (
      <div className="min-h-[calc(100vh-280px)] flex items-center justify-center px-4 py-8">
        <p className="text-text-secondary">
          Vous devez être connecté en tant que prescripteur pour accéder à cette
          page.
        </p>
      </div>
    );
        ...
        <div className="max-w-6xl mx-auto">
            ...
            <div className="mb-6 flex items-start gap-3 bg-amber-50 border-2 border-amber-300 rounded-xl px-5 py-4">
              ...
              <div>
                ...
                <p className="text-sm text-amber-800 mt-1">
                  Vous pouvez explorer toutes les fonctionnalités, mais la{" "}
                  <strong>finalisation des prescriptions</strong> sera
                  disponible une fois votre compte validé par notre équipe. Vous
              ...
              <div className="mb-6 flex items-start gap-3 bg-blue-50 border-2 border-blue-200 rounded-xl px-5 py-4">
                ...
                <div className="flex-1">
                  <p className="font-semibold text-blue-900">
                    Astuce : commencez par une ordonnance test
                  </p>
                  <p className="text-sm text-blue-800 mt-1">
                    Envoyez-vous une ordonnance à votre propre email pour voir
                    ce que votre patient recevra. Ça prend 30 secondes.
                  </p>
                </div>
                <button
          ...
          <div className="mb-8">
            ...
            <p className="text-text-secondary text-center">
              Gérez vos prescriptions, favoris et suivez les tendances de la
              communauté.
            </p>
          </div>
              ...
              {prescriptions.length === 0 ? (
                <div className="bg-white rounded-2xl border-2 border-gray-200 p-12 text-center">
                  ...
                  <p className="text-text-secondary mb-4">
                    La prochaine fois qu'un patient vous demande "quelle appli
                    me conseillez-vous ?", vous aurez la réponse en 30 secondes.
                  </p>
              ...
              {favoriteProducts.length === 0 ? (
                <div className="bg-white rounded-2xl border-2 border-gray-200 p-12 text-center">
                  ...
                  <p className="text-text-secondary mb-6">
                    Ajoutez des produits en favoris depuis le catalogue pour les
                    retrouver ici.
                  </p>
                  <button
            ...
            <div>
              {notes.length === 0 ? (
                <div className="bg-white rounded-2xl border-2 border-gray-200 p-12 text-center">
                  ...
                  <p className="text-text-secondary mb-6">
                    Ajoutez des notes depuis les fiches produit pour les
                    retrouver ici.
                  </p>
                  <button
                    onClick={() => setView("catalog")}
                ...
                <div className="space-y-3">
                  {notes.map((note) => {
                    const noteProduct = products.find(
                      (p) => p.id === note.productId,
                    );
                    const isExpanded = expandedNoteId === note.productId;
                    return (
                      <div
                        key={note.id}
                        className="bg-white rounded-2xl border-2 border-gray-200 p-6"
                        ...
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-3 mb-2">
                              {noteProduct && (
                                <button
                                  onClick={() => viewProduct(noteProduct.id)}
                            ...
                            {note.content.length > 120 && (
                              <button
                                onClick={() =>
                                  setExpandedNoteId(
                                    isExpanded ? null : note.productId,
                                  )
                                }
                                className="text-xs text-primary hover:underline mt-1 min-h-[44px] inline-flex items-center"
                              >
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <button
                              onClick={() => viewProduct(note.productId)}
                              className="inline-flex items-center gap-1.5 px-4 py-2 border-2 border-gray-200 rounded-lg text-sm font-medium text-text-secondary hover:border-primary hover:text-primary transition-colors"
                            >
                              Voir fiche
                            </button>
                            <button
                              onClick={() => handleDeleteNote(note.productId)}
                              disabled={deletingNoteId === note.productId}
                              className="inline-flex items-center gap-1.5 px-4 py-2 border-2 border-red-200 rounded-lg text-sm font-medium text-red-600 hover:bg-red-50 transition-colors disabled:opacity-50"
                            >
                              {deletingNoteId === note.productId
                                ? "..."
                                : "Supprimer"}
                            </button>
                          </div>
            ...
            <div>
              {communityStats.length === 0 ? (
                <div className="bg-white rounded-2xl border-2 border-gray-200 p-12 text-center">
                  ...
                  <p className="text-text-secondary">
                    Les statistiques apparaîtront lorsque des prescripteurs
                    commenceront à utiliser la plateforme.
                  </p>
                </div>
              ) : (
